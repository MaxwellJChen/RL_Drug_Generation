...
def _fcn_init(fcn_layer, std = np.sqrt(2), bias_const = 0.0):
    """
    Initializes layers with orthogonal weights and constant bias.
    """
...
def graph_softmax(logits, num_nodes):
    """
    Returns softmax of output probability distribution accounting for multiple graphs.
    """
...
class model(nn.Module):
    ...
    def policy(self, states, batch):
        ...
        b, p_bond = self.bond(batch, p_x, nmol, nfull)
    ...
    def act(self, states, batch, temperature=1):
        """
        ...
        t_categorical = Categorical(logits = t/temperature)
        ...
        for i, (nmol_single, nfull_single, state) in enumerate(zip(nmol_logits, nfull_logits, states)):
            nmol_categorical = Categorical(logits = nmol_single.squeeze(dim=1)/temperature)
            nmol_action = nmol_categorical.sample()
            nmol_actions += [nmol_action.item()]

            nfull_categorical = Categorical(logits = nfull_single.squeeze(dim=1)/temperature)
        ...
        b_categorical = Categorical(logits = b/temperature)
