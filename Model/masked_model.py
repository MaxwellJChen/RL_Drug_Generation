...
class masked_model(nn.Module):
    ...
    def termination(self, states, batch, p_bond, nmol, nfull, bond):
        ...
        mw = torch.tensor([Descriptors.MolWt(state) for state in states]).view(-1, 1)

        p_t = torch.cat((p_bond, bond, nmol_mean, nmol_std, nfull_mean, nfull_std, b_mean, b_std, mw), dim=1)
    ...
    def act(self, states, batch):
        ...
        t_masks = torch.vstack([M.t_mask(state) for state in states])
        t = torch.where(t_masks == 1, t, torch.tensor(-1e+8))
        ...
        for i, (nmol_single, nfull_single, state) in enumerate(zip(nmol_logits, nfull_logits, states)):
            # Nmol masking
            nmol_mask = M.nmol_mask(state).view(-1, 1)
            if 1 in nmol_mask:
                nmol_single = torch.where(nmol_mask == 1, nmol_single, torch.tensor(-1e+8))
            ...
            # Nfull masking
            nfull_mask = M.nfull_mask(state, nmol_action.item()).view(-1, 1)
            if 1 in nfull_mask:
                nfull_single = torch.where(nfull_mask == 1, nfull_single, torch.tensor(-1e+8))
        ...
        # Bond sampling
        b_masks = torch.vstack([M.b_mask(state, nmol, nfull) for state, nmol, nfull in zip(states, nmol_actions, nfull_actions)])
        b = torch.where(b_masks == 1, b, torch.tensor(-1e+8))
...
if __name__ == '__main__':
    ...
    for i in range(1, 301):
        actions, log_probs = model.act(states, batch_from_states(states))
        print(actions)
